...
/** Por qué `findCandidates` sugiere un candidato como posible duplicado. */
export type CandidateMatchReason = "dni" | "email" | "name";
...
export function findCandidates<T extends PersonForMatching>(
  ...
  pool: T[],
): Array<T & { matchReason: CandidateMatchReason }> {
  const strongMatches = new Map<string, T & { matchReason: CandidateMatchReason }>();
  if (target.nationalId) {
    for (const p of pool) {
      if (p.nationalId && p.nationalId.trim().toUpperCase() === target.nationalId.trim().toUpperCase()) {
        strongMatches.set(p.id, { ...p, matchReason: "dni" });
      }
    }
  }
  if (target.email) {
    for (const p of pool) {
      if (p.email && p.email.trim().toLowerCase() === target.email.trim().toLowerCase()) {
        if (!strongMatches.has(p.id)) strongMatches.set(p.id, { ...p, matchReason: "email" });
      }
    }
  }
  ...
  return pool
    .filter((p) => normalizeName(`${p.firstName} ${p.lastName}`) === nameKey)
    .map((p) => ({ ...p, matchReason: "name" as const }));
}
