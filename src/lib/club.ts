...
/** Lo mínimo que necesita `nextInvoiceNumber`: `db` o una transacción suya. */
 ...
 *
 * Acepta una transacción: reservar el número y escribir la factura tienen que
 * ir juntos, o un fallo al insertar quema el número para siempre.
 */
export async function nextInvoiceNumber(
  year: number,
  executor: SqlExecutor = db,
): Promise<string> {
  const rows = await executor.execute<{ last_number: number }>(sql`
