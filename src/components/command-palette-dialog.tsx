...
/**
 * El diálogo de la paleta, con todo lo que depende de `cmdk`.
 *
 * Vive en su propio fichero para que `cmdk` no entre en el bundle inicial de
 * la aplicación: `CommandPalette` lo carga con `next/dynamic` la primera vez
 * que se abre. Es el único `next/dynamic` del repo, y por eso está anotado.
 */
export default function CommandPaletteDialog({
  ...
  return (
    ...
    >
      {open ? (
        <PaletteBody permissions={permissions} close={() => onOpenChange(false)} />
      ) : null}
    </CommandDialog>
  );
}
